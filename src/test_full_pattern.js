...
// Pattern 20
const pattern20 = /(\d{1,2})分(\d{1,2})郎/;

// Test
const testText = "我是八分十五郎，猜猜我为啥在这";
const convertedText = convertChineseNumbersToArabic(testText);

console.log("Original:", testText);
console.log("Converted:", convertedText);

const match = convertedText.match(pattern20);
console.log("Pattern 20 match:", match);

if (match && match[2]) {
  const minutes = parseInt(match[1], 10);
  const seconds = parseInt(match[2], 10);
  const totalSeconds = minutes * 60 + seconds;
  console.log(`Time: ${minutes}分${seconds}秒 = ${totalSeconds}秒`);
}